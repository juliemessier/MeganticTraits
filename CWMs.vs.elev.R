...
#   2A - HERBIVORY LAYER - Rooting depth, Leaf Area, Leaf thickness weakly correlated with elevation
      # 2A.a 1970 
#     # 2A.b 2012 
...
# need to transpose plot12.dat because rows need to be plots
# need to subset species in plot12.dat to match list of species with traits
H.plot.by.sp.12<-t(plot12.dat[rownames(sp.H.traits),9:ncol(plot12.dat)])
dim(H.plot.by.sp.12) # 48 45

# Subset plot-by-species matrix to remove species with 0 abundance in 2012
H.plot.by.sp.12<-H.plot.by.sp.12[,colnames(H.plot.by.sp.12)%in%rownames(plot12.dat)]
dim(H.plot.by.sp.12) # 48 41

# Subset species-by-trait matrix to remove species with 0 abundance in 2012
H.sp.by.tr.12<-sp.H.traits[rownames(sp.H.traits)%in%rownames(plot12.dat),]
dim(H.sp.by.tr.12) # 41 7

# test compatibility of the two matrices
all(colnames(H.plot.by.sp.12)==rownames(H.sp.by.tr.12)) # TRUE

H.cwm.12<-functcomp(H.sp.by.tr.12,
                    H.plot.by.sp.12,
                    CWM.type='all')

cwm12.dat<-merge(plot.elev,H.cwm.12,by.y=0,by.x=0)
cwm12.dat$Row.names<-NULL # remove useless column entitled 'Row.names'
dim(cwm12.dat) # 48 9

# save new objects
save(H.cwm.12,file=paste0(wrk.dir,'2012.plot.cwm.herbaceous.layer.Rdata'))
save(H.plot.by.sp.12,file=paste0(wrk.dir,'plot.by.species.2012.herbaceous.layer.Rdata'))
save(H.sp.by.tr.12,file=paste0(wrk.dir,'sp.by.traits.2012.herbaceous.layer.Rdata'))
save(cwm12.dat,file=paste0(wrk.dir,'2012.plot.cwm.and.elevation.herbaceous.layer.Rdata'))

# 2 - CWM Traits vs elevation ####
#============================#
  ...
  #=====================================================================================================#
    ...
    # 1 - Scatterplots - pairwise interactions? ####
    #----
    ...
    #----
      ...
      plot(density(cwm12.dat$MeanElev)) # bimodal with peaks at 600m and 1000m
      
      # 2 - GAM - non-linearities?  ####
      #----
      
      par(mfrow=c(2,2))
      plot(gam(cwm12.dat$MeanElev~s(Ht.veg)+s(myc.frac)+
                 s(Min.Root.Loca),data=cwm12.dat))
      
      # Min.root.Loca appears curved 
      
      plot(gam(cwm12.dat$MeanElev~s(Ht.veg)+s(Lamina.thck)+
                 s(LMA),data=cwm12.dat))
      # No relationships
      
      plot(gam(cwm12.dat$MeanElev~s(Leaf.Area),data=cwm12.dat))
      # No relationships
      ...
      summary(m8<-lm(cwm12.dat$Ht.veg~MeanElev+I(MeanElev^2),data=cwm12.dat)) # NS
      
      # Min.Root.Loca
      
      summary(m9<-lm(cwm12.dat$Min.Root.Loca~MeanElev+I(MeanElev^2),data=cwm12.dat)) # Significant
      
      # Coefficients:
      #             Estimate Std. Error t value Pr(>|t|)  
      # (Intercept)   -1.705e-02  1.281e+00  -0.013   0.9894  
      # MeanElev       8.757e-03  3.511e-03   2.494   0.0164 *
      # I(MeanElev^2) -5.935e-06  2.302e-06  -2.578   0.0133 *
      # ---
      # Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
      # 
      # Residual standard error: 0.4606 on 45 degrees of freedom
      # Multiple R-squared:  0.1347,	Adjusted R-squared:  0.09623 
      # F-statistic: 3.502 on 2 and 45 DF,  p-value: 0.03859
      
      plot(cwm12.dat$Min.Root.Loca~MeanElev,data=cwm12.dat,
           xlab='Plot elevation (m)', ylab='Rooting Depth CWM',family='serif',pch=20)
      points(x=cwm12.dat$MeanElev, 
             y=fitted(m9),
             col='red', pch=20)
      text(900,3.8,labels= 'y= 8e-03x -6e-06 x^2 \n AdjR2=0.10, p=0.04',family='serif',cex=0.8)
      
      # Lamina.thck
      
      summary(m10<-lm(cwm12.dat$Lamina.thck~MeanElev, data=cwm12.dat)) # NS
      ...
      summary(m13<-lm(cwm12.dat$Leaf.Area~MeanElev,data=cwm12.dat)) # N.S.
      
      # myc.frac
      
      summary(m14<-lm(cwm12.dat$myc.frac~MeanElev+I(MeanElev^2),data=cwm12.dat)) # NS
